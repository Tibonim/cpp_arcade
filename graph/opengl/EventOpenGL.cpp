...
namespace arcade {
  ...
  void	GraphOpenGL::push(int ch) const {
  ...
  int	GraphOpenGL::getShiftKey(int ch) const {
  ...
  void	GraphOpenGL::pushMyEvents() const
  {
  ...
  void	GraphOpenGL::unstackKeyEvent(int event)
  {
    _key_event.erase(std::remove(_key_event.begin(), _key_event.end(), event),
		     _key_event.end());
  }

  void	GraphOpenGL::pushKeyEvent(int event)
  {
    auto it = _key_event.begin();
  ...
  int	GraphOpenGL::keyHook(GraphOpenGL::broadCmd_t const&)
  {
    int	nb = -1;
